...
const createTodoForm = () => {
  ...
  input.classList.add('form-control', 'rounded-2', 'me-1');
...
const createTodoItem = (name) => {
  ...
  const randomId = Math.random() * 13.33;
  ...
  doneBtn.classList.add('btn', 'btn-success', 'me-1');
  ...
  return { todoItem, doneBtn, deleteBtn, btnWrapper };
};
...
function createTodoApp(container, title, key) {
  ...
  appForm.form.addEventListener('submit', (event) => {
    ...
    let localeStorageData = localStorage.getItem(key);

    if (localeStorageData === null) {
      todoArray = [];
    } else {
      todoArray = JSON.parse(localeStorageData);
    }
